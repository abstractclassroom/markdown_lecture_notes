...
The number of vertices is represented by $|V|$ and the number of edges is $|E|$. 
...
Here are some properties of the adjacency matrix.

* The adjacency matrix is always symmetric (matrix equal to its transpose) for an undirected graph.
* A simple graph will always have zeros down the main diagonal (top left to bottom right)
* The number of edges in a directed graph will equal the number of nonzero entries in the adjacency matrix. 
* The number of edges in a simple undirected graph will always be half the number of nonzero entries in the adjacency matrix. This is due to the implied symmetry of the relation.
...
Now consider matrix $A$ raised to the tenth power, denoted $A^{10}$ (matrix $A$ multiplied by itself 10 times).
