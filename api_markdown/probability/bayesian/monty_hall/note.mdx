...
If contestants go into the game knowing the facts above they would always switch doors.  So an observer of the game would see contestants win  two thirds of the time.  However, if we assume all contestants are unaware that switching doors improves their chance of winning then they will be just as likely to keep their original door choice as they are to switch.  

With the unaware contestants assumption, observers will see contestants win the game with a probability of $\frac{1}{2}$.
...
* Observers watching unaware contestants will see the game won with a probability of $\frac{1}{2}$.
...
* An observer watching unaware contestants see then win half the time.
...
At this point, we have described the probabilities facing the contestant. Now, let's consider the probabilities associated with an observer watching games that entail unaware contestants. The observer probability is expressed below.
...
Substituting the probabilities calculated earlier and the fact constestants are unaware:
...
- $P(D_s = D_p) = \frac{1}{2}$ (probability of not switching by unaware contestant),
- $P(D_s \neq D_p) = 1- P(D_s = D_p) = \frac{1}{2}$ (probability of switching by unaware contestant),
...
P(D_s = D_c) &= \underbrace{P(D_s = D_c \mid D_s = D_p)}_{\frac{1}{3}}\underbrace{P(D_s = D_p)}_{\frac{1}{2}} 
 + \frac{P(D_s = D_c \mid D_s \neq D_p)}_{\frac{2}{3}} \underbrace{P(D_s \neq D_p)}_{\frac{1}{2}} \\\\
&= \frac{1}{3} \cdot \frac{1}{2} + \frac{2}{3} \cdot \frac{2}{3} \\\\
&= \frac{1}{6} + \frac{2}{6} \\\\
&= \frac{3}{6} \\\\
&= \frac{1}{2}
\end{align*}
...
Thus, an observer watching games played by unaware contestants sees the game won with a probability of \(\frac{1}{2}\). This result demonstrates how the observer's perspective combines the contestants' decisions into a simple average outcome.
