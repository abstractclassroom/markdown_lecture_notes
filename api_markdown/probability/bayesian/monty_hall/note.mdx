...
- $\underbrace{(D_m = D_c)}_{\bot} \implies P(D_m = D_c) = 0$
- $\underbrace{(D_m = D_p)}_{\bot} \implies P(D_m = D_p) = 0$
- $\underbrace{P(D_m \neq D_p) \land (D_m \neq D_c)}_{\top} \implies P(D_m \neq D_p \land D_m \neq D_c) = 1$
...
\begin{align*}
& P(D_s = D_c | D_s = D_p) \\\\
    &= P(D_s = D_c | D_s = D_p, D_m = D_c) \cdot \underbrace{P(D_m = D_c)}_{0} \\\\
        &\quad \quad+ P(D_s = D_c | D_s = D_p, D_m = D_c) \cdot \underbrace{P(D_m = D_p)}_{0} \\\\
        &\quad\quad+ P(D_s = D_c | D_s = D_p, D_m \neq D_p \land D_m \neq D_c) \cdot \underbrace{P(D_m \neq D_p \land D_m \neq D_c)}_{1}
\end{align*}
$$
...
& P(D_s = D_c | D_s = D_p) \\\\
    & \quad= P(D_s = D_c | D_s = D_p, D_m \neq D_p \land D_m \neq D_c) \\
    & \quad= P(\underbrace{D_s = D_c \land D_s = D_p}_{D_c = D_p} \land \underbrace{D_m \neq D_p \land D_m \neq D_c}_{\top}) \\\\
    & \quad = P(D_c = D_p)\\
    & \quad = \frac{1}{3}
\end{align*}
$$
...
Next, let’s consider the probability of the contestant winning the car by switching doors:
...
&P(D_s = D_c | D_s \neq D_p) \\\\
    &\quad = P(D_s = D_c | D_s \neq D_p, D_m \neq D_p \land D_m \neq D_c) \\
    &\quad = P(\underbrace{D_s = D_c \land D_s \neq D_p}_{D_c \neq D_p} \land \underbrace{D_m \neq D_p \land D_m \neq D_c}_{\top}) \\\\
    &\quad = P(D_c \neq D_p) \\
    &\quad = 1 - P(D_c = D_p) \\
    &\quad = 1 - \frac{1}{3} \\
    &\quad = \frac{2}{3}
...
\begin{align*}
&P(D_s = D_c) \\
    &\quad= P(D_s = D_c \mid D_s = D_p) \cdot P(D_s = D_p) \\
    &\quad \quad + P(D_s = D_c \mid D_s \neq D_p) \cdot P(D_s \neq D_p) 
\end{align*}
$$
...
\begin{align*}
&P(D_s = D_c) \\\\ 
    &= \quad\underbrace{P(D_s = D_c \mid D_s = D_p)}_{\frac{1}{3}} \cdot \underbrace{P(D_s = D_p)}_{\frac{1}{2}} \\\\
        &\quad \quad+ \underbrace{P(D_s = D_c \mid D_s \neq D_p)}_{\frac{2}{3}} \cdot \underbrace{P(D_s \neq D_p)}_{\frac{1}{2}} 
\end{align*}
$$

We have the final result for the probability value.

$$P(D_s = D_c) = \frac{1}{3} \cdot \frac{1}{2} + \frac{2}{3} \cdot \frac{1}{2} = \frac{1}{2}$$
