...
1. Gets the relevant value from the initial permutation table.
2. 64 - ip_table[$i$] calculates the bit position that will be swapped.
3. Right shifts the bits of $x$ to put it in the least significant bit position.
4. AND 1 effectively gets the value of the bit in the least significant bit position.
5. That least significant bit is shift over to position $i$.
6. The value is then bitwise OR with the running result.
